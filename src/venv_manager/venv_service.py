...
class VenvService:
    ...
    def create_venv(self, root: Path, name: str, base_python: str) -> Path:
        ...
        result = self._run_background(command)
    ...
    def check_base_python(self, base_python: str) -> bool:
        try:
            command_parts = self._split_base_python(base_python)
            if self._is_non_python_executable_command(command_parts[0]):
                return False
            command = [*command_parts, "--version"]
            result = self._run_background(command)
    ...
    def get_python_version(self, python_executable: Path) -> str:
        if not python_executable.exists():
            return "未知"
        if not self._looks_like_python_executable(python_executable):
            return "未知"
        result = self._run_background(
    ...
    def _discover_from_py_launcher(self) -> list[PythonInstallation]:
        if sys.platform != "win32":
            return []
        try:
            result = self._run_background(["py", "-0p"])
        except OSError:
            return []
        if result.returncode != 0:
            return []
    ...
    def _run_background(self, command: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    def _looks_like_python_executable(self, path: Path) -> bool:
        name = path.name.lower()
        if sys.platform == "win32":
            return name in {"python.exe", "pythonw.exe"}
        return name.startswith("python")

    def _is_non_python_executable_command(self, command: str) -> bool:
        path = Path(command)
        if path.suffix.lower() != ".exe" or not path.exists():
            return False
        return path.name.lower() not in {"python.exe", "pythonw.exe", "py.exe"}
